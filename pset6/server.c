...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // start of line = start of method
            char* method = line;
            // find first space
            char* requestTarget = strchr(line, ' ');
            // if not found, error and continue
            ...
            // change it to '\0'
            *requestTarget = '\0';
            // move it one space to the right so it points to request-target's start
            requestTarget++;
            
            // find second space (now we start from request target)
            char* httpVersion = strchr(requestTarget, ' ');
            if (httpVersion == NULL)
            {
                error(400);
                continue;
            ...
            // change it to '\0'
            *httpVersion = '\0';
            // move it one space to the right so it points to HTTP-ver's start
            httpVersion++;
            ...
            char* queryStart = strchr(requestTarget,'?');
            int queryLength;
            if (queryStart == NULL)
            {
                queryLength = 0;
            ...
            {
                // in line change ? to '\0' to have absolute path
                *queryStart = '\0';
                // now requestTarget is absolute-path       
                // avoid the ? and get length
                queryStart++;
                queryLength = strlen(queryStart);
            }
            ...
            {
                strncpy(query, queryStart, queryLength);
                query[queryLength] = '\0';
            }
            ...
            int reqTarLength = strlen(requestTarget);
            int rootLength = strlen(root);
            
            // concatenate root and absolute-path
            char path[reqTarLength + rootLength + 1];
            strcpy(path, root);
